    ...
    this.draw = () => {
        ...
        if((this.startingPoint.x - this.radius) < canvas.width - 65 && (this.startingPoint.x + this.radius) >  65){
            if(mousePos.x < canvas.width - 65 && mousePos.x >  65){
                ...
                pushToCheckPointTracker({checked: inside, label: this.label, overlap, canClick: false});
            }
        }else{
            overlap = true;
            pushToCheckPointTracker({checked: false, label: this.label, overlap, canClick: true});
        }
        if(this.selected){
            ctx.fillStyle = 'gold'; ctx.globalAlpha = 0.5;
        }else{
